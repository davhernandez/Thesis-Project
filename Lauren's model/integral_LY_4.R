...
hist(gopher_sizeobs_F0, breaks = 100, xlab = "Size (cm)", main = "Blue F=0", ylim = c(0,15))

# F=0.19 (Pt. Lobos amount) -------------------------------
...
parms=c(k=0.172, M=0.14, Linf=38.15,Fi=0.0);
...
colnames(N_mean_size) <- c("mean_size", "N_mean_size")
gopher_sizes <- left_join(gopher_sizes, N_mean_size, by = "mean_size")
gopher_sizes[is.na(gopher_sizes)] <- 0
...
hist(gopher_sizeobs_F0.05, breaks = 50, xlab = "Size (cm)", main = "Blue F=0.19", ylim =c(0,100))
